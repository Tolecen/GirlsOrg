...
- (id)initWithFrame:(CGRect)frame
{
    if (self = [super initWithFrame:frame]) {
        self.pullToRefreshText = CommonLocalizedStrings(@"pullToRefresh");
        self.releaseToRefreshText = CommonLocalizedStrings(@"releaseToRefresh");
        self.refreshingText = CommonLocalizedStrings(@"hearderRefresh_title");
    }
    return self;
}
...
- (void)updateTimeLabel
{
    ...
    // 3.显示日期
    self.lastUpdateTimeLabel.text = [NSString stringWithFormat:@"%@%@", CommonLocalizedStrings(@"headerLastUpdated"),time];
}
